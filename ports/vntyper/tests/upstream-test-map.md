...
- Runtime tests executing BioScript VNtyper programs: covered by
  `rust/bioscript-runtime/tests/vntyper_program.rs`. `vntyper.bs` and
  `vntyper-bam-native.bs` run native Samtools/Kestrel/BCFtools/VCF facades on
  tiny BAM fixtures. `vntyper-fastq.bs` runs native Kestrel/BCFtools/VNtyper
  Kestrel call-table parsing on tiny generated FASTQ/reference fixtures, writes
  `kestrel_result.tsv` plus a TSV summary, and materializes report JSON through
  the VCF facade. Full TSV/JSON/HTML parity remains open.
