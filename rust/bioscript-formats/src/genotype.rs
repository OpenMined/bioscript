...
mod tests {
    ...
    use crate::genotype::{
        ...
        io::{
            detect_source_format, is_bgzf_path, looks_like_vcf_lines, read_plain_lines,
            select_zip_entry,
        },
        types::{DelimitedBackend, VcfBackend},
        vcf::{
    ...
    #[test]
    fn genotype_loads_gzipped_text_from_path_and_bytes() {
        let text = b"rsid\tgenotype\nrs1\tAG\n";
        let compressed = gzip_bytes(text);

        let dir = temp_dir("gzipped-text");
        let path = dir.join("sample.txt.gz");
        fs::write(&path, &compressed).unwrap();
        let from_path = GenotypeStore::from_file(&path).unwrap();
        assert_eq!(from_path.get("rs1").unwrap().as_deref(), Some("AG"));

        let from_bytes = GenotypeStore::from_bytes("sample.txt.gz", &compressed).unwrap();
        assert_eq!(from_bytes.get("rs1").unwrap().as_deref(), Some("AG"));
    }

    #[test]
    fn genotype_loads_vcf_gz_entry_from_zip_path() {
        let dir = temp_dir("zip-vcf-gz");
        let zip_path = dir.join("sample.zip");
        let mut vcf_gz = Vec::new();
        {
            let mut writer = bgzf::io::Writer::new(&mut vcf_gz);
            writer
                .write_all(
        ...
        }
        {
            ...
            archive
                ...
                .unwrap();
            archive.write_all(&vcf_gz).unwrap();
