...
fn map_bam_op(op: noodles::sam::alignment::record::cigar::Op) -> AlignmentOp {
    use noodles::sam::alignment::record::cigar::op::Kind;
...
pub(super) fn record_overlaps_locus(record: &AlignmentRecord, locus: &GenomicLocus) -> bool {
    record.end >= locus.start && record.start <= locus.end
}

pub(super) fn spans_position(record: &AlignmentRecord, pos: i64) -> bool {
