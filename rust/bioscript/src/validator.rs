    ...
    if let Some(canonical_alt) = scalar_at(root, &["alleles", "canonical_alt"]) {
        ...
        if !alts.iter().any(|alt| alt == &canonical_alt) {
            issues.push(Issue {
                ...
                message: format!(
                    "canonical_alt '{}' is not present in alleles.alts {:?}",
                    canonical_alt, alts
                ),
            });
        }
...
fn validate_kind_vs_tags(root: &Value, issues: &mut Vec<Issue>) {
    ...
    let has_snp = tags.iter().any(|tag| tag == "snp");
    let has_snv = tags.iter().any(|tag| tag == "snv");
    if kind == "snv" && has_snp && !has_snv {
        issues.push(Issue {
            severity: Severity::Warning,
...
fn validate_pgx_shape(root: &Value, issues: &mut Vec<Issue>) {
    ...
    for key in ["drug_labels", "annotations", "clinical_annotations"] {
        let Some(items) = pgx.get(Value::String(key.to_owned())).and_then(Value::as_sequence) else {
            continue;
        };
