...
# VNtyper-correct Kestrel configuration. These mirror upstream
# vntyper/scripts/kestrel_config.json for the original Java VNtyper path.
KMER_SIZE = 20
MIN_KMER_COUNT = 5
MAX_HAPLOTYPES = 40
MAX_SAVED_STATES = 40
...
def write_placeholder(path, description):
    bioscript.write_text(
        path,
        "BioScript VNtyper placeholder artifact\n"
        + description
        + "\nThis path is emitted for upstream folder-shape parity; semantic "
        + "parity is still tracked in vntyper-todo.md.\n",
    )
...
def main():
    ...
    bioscript.copy_file(sliced_bam, "/output/kestrel/output.bam")
    bioscript.copy_file(sliced_bai, "/output/kestrel/output.bam.bai")
    write_placeholder(
        "/output/kestrel/output_indel.vcf.gz",
        "bgzip-compressed indel VCF is not generated by the current native BioScript path.",
    )
    write_placeholder(
        "/output/kestrel/output_indel.vcf.gz.csi",
        "CSI index for bgzip-compressed indel VCF is not generated by the current native BioScript path.",
    )
    bioscript.write_text(
        "/output/kestrel/output.bed",
        "chr1\t155158000\t155163000\tMUC1\n",
    )
    bioscript.write_text("/output/kestrel/kestrel_kmer_20.log", "bioscript kestrel.run_native\n")
    bioscript.write_text("/output/kestrel/samtools_view.log", "bioscript placeholder: Kestrel SAM to BAM conversion not generated\n")
    bioscript.write_text("/output/kestrel/samtools_index.log", "bioscript placeholder: Kestrel BAM index copied from sliced BAM index\n")
    bioscript.write_text("/output/kestrel/bcftools_sort.log", "bioscript placeholder: bgzip/sort/index not generated\n")
    bioscript.write_tsv(
    ...
    bioscript.copy_file(sliced_bam, "/output/fastq_bam_processing/output_unmapped.bam")
    write_placeholder(
        "/output/fastq_bam_processing/output_other.fastq.gz",
        "samtools fastq -0 equivalent output is not generated by the current native BioScript path.",
    )
    write_placeholder(
        "/output/fastq_bam_processing/output_single.fastq.gz",
        "samtools fastq -s equivalent output is not generated by the current native BioScript path.",
    )
    ...
    bioscript.write_text(
        "/output/README.md",
        "# BioScript VNtyper output\n\n"
        + "This directory follows the upstream VNtyper output folder shape. "
        + "Some artifacts are placeholders until the native BioScript port "
        + "implements the corresponding samtools, Kestrel, and bcftools steps.\n",
    )
    bioscript.write_text(
        "/output/pipeline.log",
